...
def split_survey_data(survey_data: List[Dict], demo_size: int = 10, validation_size: int = 10, seed: int = 0) -> Tuple[List[Dict], List[Dict]]:
    ...
    Args:
        ...
        validation_size: Number of validation examples (default: 10)
...
def apply_context_to_dataset(dataset_data: List[Dict], context: List[Dict]) -> List[Dict]:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Random seed for splitting survey data. Default: 0"
    )
    ...
    survey_path4/survey_100.jsonl"
    output_dir = f"/hpc/group/fanglab/xx102/vpl_llm/inference_results_dem_val_context_subset_{data_subset}"

    # Demo and validation split parameters
    demo_size = 50
    validation_size = 50
    ...
    # Load survey data and split into demo and validation
    print("\n=== Loading Survey Data ===")
    survey_data = load_survey_data(survey_path)
    print(f"Loaded {len(survey_data)} survey samples")

    print("\n=== Splitting Survey Data ===")
    demo_data, validation_data = split_survey_data(survey_data, demo_size=demo_size, validation_size=validation_size, seed=args.seed)
    print(f"Demo samples: {len(demo_data)}")
    print(f"Validation samples: {len(validation_data)}")

    # Combine all demo and validation data as context (20 samples total)
    ...
    print(f"Using {len(best_context)} context samples for inference")
